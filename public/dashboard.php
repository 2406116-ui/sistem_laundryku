...
$kritis = $conn->query("SELECT t.id_transaksi, p.nama_pelanggan, t.tanggal_masuk, t.berat_kg, t.jenis
                        ...
                        WHERE t.status = 'proses' ORDER BY t.tanggal_masuk ASC LIMIT 5");
...
<body>
    ...
    <div class="main-content">
        ...
        <?php if ($jumlahKritis > 0): ?>
            <div class="alert">⚠️ Terdapat <?= $jumlahKritis ?> transaksi yang masih dalam status PROSES. Segera selesaikan!</div>
        <?php endif; ?>
        ...
        <div class="two-columns">
            <div class="panel">
                ...
                <?php if ($kritis && $kritis->num_rows > 0): ?>
                    <ul><?php while ($row = $kritis->fetch_assoc()): ?><li><span><strong><?= htmlspecialchars($row['nama_pelanggan']) ?></strong><br><small><?= $row['jenis'] ?> - <?= $row['berat_kg'] ?> kg</small></span><span class="badge-warning"><?= date('d M Y', strtotime($row['tanggal_masuk'])) ?></span></li><?php endwhile; ?></ul>
                <?php else: ?><p>Tidak ada transaksi dalam proses.</p><?php endif; ?>
            </div>
            <div class="panel">
                <h3>🕒 Aktivitas Terbaru</h3>
                <?php if ($aktivitas && $aktivitas->num_rows > 0): ?>
                    <ul><?php while ($row = $aktivitas->fetch_assoc()): ?><li><span><?= htmlspecialchars($row['nama_pelanggan']) ?> - <?= ucfirst($row['status']) ?></span><span><?= date('d M Y', strtotime($row['tanggal_masuk'])) ?></span></li><?php endwhile; ?></ul>
                <?php else: ?><p>Belum ada aktivitas transaksi.</p><?php endif; ?>
            </div>
        </div>
        <div class="panel">
            ...
            <?php else: ?><div class="item-detail">Tidak ada data.</div><?php endif; ?>
        </div>
        <div class="footer">Administrator | LaundryKu v1.0</div>
    </div>
</body>
